    ...
    <article class="kb-card selectable ${item.id === selectedKnowledgeBaseId ? 'active' : ''}" data-kb-id="${escapeHtml(item.id)}">
      <div class="row">
        ...
        <div class="action-stack">
          <button type="button" class="ghost small-btn" data-open-kb="${escapeHtml(item.id)}">打开知识库所在位置</button>
          <button type="button" class="danger small-btn" data-delete-kb="${escapeHtml(item.id)}">删除知识库</button>
        </div>
      </div>
    </article>
  `).join('');
...
async function loadDocuments(showOverlay = false) {
  ...
  try {
    ...
    docListEl.innerHTML = data.items.map(doc => `
      <article class="kb-card doc-card">
        <div class="row">
          ...
          <div class="action-stack">
            <button class="ghost small-btn" data-open-doc="${escapeHtml(doc.id)}">打开所在位置</button>
            <button class="danger small-btn" data-delete-doc="${escapeHtml(doc.id)}">删除文档</button>
          </div>
        </div>
      </article>
    `).join('');
...
uploadForm.addEventListener('submit', async event => {
  ...
  try {
    ...
    await withBusy('正在上传并处理文档，请稍等...', () => requestJson('/api/documents', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
