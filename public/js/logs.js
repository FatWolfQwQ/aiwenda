...
async function loadLogs(showOverlay = false) {
  ...
  try {
    ...
    const logItems = toArray(data.items);
    if (!logItems.length) {
          ...
          <article class="kb-card selectable log-card" data-log-id="${escapeHtml(item.id)}">
            <div class="row log-row">
              ...
              <div class="log-card-actions">
                ...
                ${isServerMode() ? `<button class="ghost small-btn" data-download-log="${escapeHtml(item.id)}">下载日志</button>` : `<button class="ghost small-btn" data-open-log="${escapeHtml(item.id)}" ${item.exported ? '' : 'disabled'}>打开位置</button>`}
              </div>
            </div>
          </article>
        `).join('')}
...
logListEl.addEventListener('click', async event => {
  ...
  const downloadButton = event.target.closest('[data-download-log]');
  if (downloadButton) {
    event.stopPropagation();
    openDownload(`/api/logs/${encodeURIComponent(downloadButton.dataset.downloadLog)}/download`);
    return;
  }
