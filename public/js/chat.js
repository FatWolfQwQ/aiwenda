  ...
  return citations.map(item => `
    <article class="citation-card">
      <div class="citation-head">
        <strong>${escapeHtml(item.filename)} · 资料依据 ${item.chunkIndex}${showRelevance ? ` · 相关度 ${Math.round(item.score * 100)}%` : ''}</strong>
      </div>
...
async function openChatHistory() {
  ...
  try {
    ...
    chatHistoryList.innerHTML = data.items.length ? data.items.map(item => `
